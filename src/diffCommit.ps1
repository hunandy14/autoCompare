function Invoke-Git {
    ...
    try {
        ...
        Write-Verbose "Using Git repository at: $resolvedPath"
...
Invoke-Git show not_exist_branch -EA Stop -Verbose
...
function diffCommit {
    ...
    # 執行命令獲取差異資訊
    $results = @{
        Status    = @(Invoke-Git diff --name-status $gitParams.Filter $gitParams.Stage $Commit1 $Commit2 -Path $Path)
        NumStat   = @(Invoke-Git diff --numstat $gitParams.Filter $gitParams.Stage $Commit1 $Commit2 -Path $Path)
        Untracked = @(if (!$Tracked -and (!$Cached -and !$Commit2)) { 
            @(Invoke-Git ls-files --others --exclude-standard -Path $Path) | 
